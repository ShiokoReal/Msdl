...
namespace Me.Shishioko.Msdl.Test
{
    public sealed class Game
    {
        ...
        public Game()
        {
            World worldDefault = new(this, new("world:default", 256, 0)
            {
                Effects = Data.Sky.Overworld,
                HasSkylight = false,
            });
            Worlds.Add(worldDefault.Name, worldDefault);
        ...
        internal async Task ServeAsync(Player player)
        {
            while(player.Connection.ProtocolState != ProtocolState.Disconnected)
            {
                ...
                try
                {
                    ...
                    {
                        await BroadcastMessageAsync(new ChatText("")
                        {
                            Extra = [
                    ...
                    _ = BroadcastMessageAsync(new ChatText($"{player.Name} joined")
                    {
                        Color = player.DarkColor,
                        Italic = true,
                    });
                ...
                finally
                {
                    ...
                    if (Reloader is null)
                    {
                        _ = BroadcastMessageAsync(new ChatText($"{player.Name} left")
                        {
                            Color = player.DarkColor,
                            Italic = true,
                        });
