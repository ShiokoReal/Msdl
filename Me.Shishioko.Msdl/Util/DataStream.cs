...
namespace Me.Shishioko.Msdl.Util
{
    public sealed class DataStream : IDisposable
    {
        ...
        public void WriteU16A(ReadOnlySpan<ushort> data)
        {
            ...
            byte[] span = new byte[data.Length * sizeof(ushort)];
        ...
        public void WriteS16A(ReadOnlySpan<short> data)
        {
            ...
            byte[] span = new byte[data.Length * sizeof(short)];
            for (int i = 0; i < data.Length; i++)
            {
        ...
        public void WriteU32A(ReadOnlySpan<uint> data)
        {
            ...
            byte[] span = new byte[data.Length * sizeof(uint)];
            for (int i = 0; i < data.Length; i++)
            {
        ...
        public void WriteS32A(ReadOnlySpan<int> data)
        {
            ...
            byte[] span = new byte[data.Length * sizeof(int)];
            for (int i = 0; i < data.Length; i++)
            {
        ...
        public void WriteU64A(ReadOnlySpan<ulong> data)
        {
            ...
            byte[] span = new byte[data.Length * sizeof(ulong)];
            for (int i = 0; i < data.Length; i++)
            {
        ...
        public void WriteS64A(ReadOnlySpan<long> data)
        {
            ...
            byte[] span = new byte[data.Length * sizeof(long)];
            for (int i = 0; i < data.Length; i++)
            {
        ...
        public void WriteF32A(ReadOnlySpan<float> data)
        {
            ...
            byte[] span = new byte[data.Length * sizeof(float)];
            for (int i = 0; i < data.Length; i++)
            {
        ...
        public void WriteF64A(ReadOnlySpan<double> data)
        {
            ...
            byte[] span = new byte[data.Length * sizeof(double)];
            for (int i = 0; i < data.Length; i++)
            {
        ...
        {
            Contract.Requires(CanWrite);
            Stream.Write(MemoryMarshal.AsBytes<sbyte>(new sbyte[] { data }));
        }
        ...
        {
            Contract.Requires(CanWrite);
            Stream.Write(MemoryMarshal.AsBytes<bool>(new bool[] { data }));
        }
        public void WriteGuid(Guid data)
        {
            Contract.Requires(CanWrite);
            byte[] buffer = MemoryMarshal.AsBytes<Guid>([data]).ToArray(); //TODO: replace all array initialization syntax with this
            if (!BitConverter.IsLittleEndian)
            {
                Stream.Write(buffer);
                return;
            }
            Stream.Write(new byte[] { buffer[3], buffer[2], buffer[1], buffer[0], buffer[5], buffer[4], buffer[7], buffer[6], buffer[8], buffer[9], buffer[10], buffer[11], buffer[12], buffer[13], buffer[14], buffer[15] });
        }
        ...
        public Guid ReadGuid()
        {
            ...
            if (BitConverter.IsLittleEndian) buffer = new byte[] { buffer[3], buffer[2], buffer[1], buffer[0], buffer[5], buffer[4], buffer[7], buffer[6], buffer[8], buffer[9], buffer[10], buffer[11], buffer[12], buffer[13], buffer[14], buffer[15] };
